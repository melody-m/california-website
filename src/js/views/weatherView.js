...
function getDaysName(){
    ...
    let tomorrowNum = new Date(new Date().getTime() + 24 * 60 * 60 * 1000).getDay();
    let nextNum = new Date(new Date().getTime() + 48 * 60 * 60 * 1000).getDay();

    return [week[todayNum], week[tomorrowNum], week[nextNum]];
}
...
export function displayIcon(arr, container){
    ...
    daysArr.forEach((cur, i)=>{
        ...
        cur.insertAdjacentHTML(
            'afterbegin',
            `<div class="weather__icon"><img src="${imgArr[i]}" alt="" class="weather__icon--img"></div>`
        );
    })
}
